predict_gam <- function (object, newdata, type = "link", se.fit = FALSE, terms = NULL, 
          ...
          ...) 
{
  ...
  if (type != "link" && type != "terms" && type != "iterms" && 
      type != "response" && type != "lpmatrix" && type != 
      "newdata") {
      ...
      if (n.smooth) 
        for (k in 1:n.smooth) {
          ...
          if ((is.null(terms) || (klab %in% terms)) && 
            ...
            Xfrag <- PredictMat(object$smooth[[k]], 
                                data)
                ...
                se[start:stop, n.pterms + k] <- sqrt(pmax(0, 
                                                          rowSums((X1 %*% object$Vp) * X1)))
                                                             ...
                                                             rowSums((X[, first:last, drop = FALSE] %*% 
                                                                        object$Vp[first:last, first:last, 
                                                                                  drop = FALSE]) * X[, first:last, 
                                                                                                     drop = FALSE])))
            }
          }
  ...
  if (type == "lpmatrix") {
    ...
    if (!is.null(offs)) {
      offs <- offs[1:nlp]
      for (i in 1:nlp) offs[[i]] <- napredict(na.act, 
                                              offs[[i]])
